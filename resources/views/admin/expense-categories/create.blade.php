...
@section('title', ln('Create Expense Category', 'ব্যয় বিভাগ তৈরি', '创建费用类别'))
@section('content')
    <div class="card p-4">
        <h4 class="mb-3"> {{ ln('Create Expense Category', 'ব্যয় বিভাগ তৈরি', '创建费用类别') }}</h4>
        ...
        <form method="POST" action="{{ route('admin.expense-categories.store') }}">
            ...
            <button class="btn btn-primary"> {{ ln('Save', 'সংরক্ষণ', '保存') }}</button>
        </form>
    </div>
@endsection
